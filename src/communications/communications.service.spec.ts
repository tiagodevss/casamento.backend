...
function group(overrides: Record<string, unknown> = {}) {
  return {
    ...
    createdAt: new Date(),
    updatedAt: new Date(),
    members: [{ id: 'member-1', name: 'João', isChild: false, attending: true, sortOrder: 0, createdAt: new Date(), updatedAt: new Date(), guestGroupId: 'group-1' }],
    rsvpResponse: { id: 'rsvp-1', guestGroupId: 'group-1', partyAttending: null, diet: null, message: null, respondedIp: null, respondedAt: new Date(), updatedAt: new Date() },
    ...overrides,
  } as any;
}
...
describe('CommunicationsService eligibility', () => {
  ...
  const evaluate = (guest: any, audience: CommunicationAudience) =>
    (service as any).evaluateEligibility(guest, campaign(audience));
  ...
  it('never exposes party details to a party invite that declined the reception', () => {
    ...
    expect(evaluate(declinedParty, CommunicationAudience.CEREMONY_ONLY_CONFIRMED)).toEqual({ eligible: true });
  ...
  it('partitions a confirmed party guest into the party communication only', () => {
    ...
    expect(evaluate(partyGuest, CommunicationAudience.PARTY_CONFIRMED)).toEqual({ eligible: true });
  ...
  it('blocks opt-out and invalid phones before audience rules', () => {
    ...
    expect(evaluate(group({ phone: '123', phoneNormalized: null }), CommunicationAudience.ALL)).toEqual({
      eligible: false,
